...
export default function RowMeili({ titulo, repositorioId, verMasTo, perPage = 12, paginate }: Props) {
  ...
  React.useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const res: RecursosResponse = await getRecursosWithMeilisearch(1, perPage, [repositorioId], "", "title", "views", "desc");
        setItems(res.data);
      } finally {
        setLoading(false);
      }
